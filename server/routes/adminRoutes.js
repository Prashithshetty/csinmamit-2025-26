 ...
 */
router.post('/send-otp', otpLimiter, validateRequest(['email', 'name']), async (req, res) => {
  try {
 ...
 */
router.post('/verify-otp', verifyLimiter, validateRequest(['email', 'otp']), async (req, res) => {
  try {
 ...
 */
router.post('/resend-otp', otpLimiter, validateRequest(['email', 'name']), async (req, res) => {
  try {
    const { email, name } = req.body
    ...
    console.log(`Admin OTP resent to: ${email} at ${new Date().toISOString()}`)
